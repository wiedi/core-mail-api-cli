...
class CatchAllTests(unittest.TestCase):
	...
	def setUpClass(cls):
		createBasicLayout()
		api.createMailAlias('*', CONFIG['test_domain'], 'acc2@' + CONFIG['test_domain'])

	@classmethod
	def tearDownClass(cls):
		removeBasicLayout()
		api.deleteMailAlias('*', CONFIG['test_domain'])
	...
	def test_catchall(self):
		token = mail.send(
			to        = ['does-not-exist@' + CONFIG['test_domain']],
			from_addr = 'acc1@' + CONFIG['test_domain'],
			server    = CONFIG['submission_server'],
			port      = '587',
			user      = 'acc1@' + CONFIG['test_domain'],
